...
const USER_INCLUDE = { shop: true, role: true, permissionOverrides: true, registrationRequest: true };
 ...
 */
const startTrialOnLogin = async (userRecord) => {
  if (userRecord && userRecord.shop && !userRecord.shop.planExpiresAt) {
    const plan = userRecord.shop.planId 
      ? await db.subscriptionPlan.findUnique({ where: { id: userRecord.shop.planId } })
      : null;
    ...
    const updatedShop = await db.shop.update({
      where: { id: userRecord.shop.id },
      data: { planExpiresAt: expiresAt },
    });
    userRecord.shop = updatedShop;
  }
  return userRecord;
};
...
export const checkUser = async () => {
  try {
    ...
    if (existing) {
      ...
      return await startTrialOnLogin(existing);
    }
    ...
    if (invitation) {
      ...
      if (expired) {
        ...
        return await startTrialOnLogin(await db.user.findUnique({ where: { id: created.id }, include: USER_INCLUDE }));
      }
    }

    // C. Brand-new self-signup → create user only, no shop
    let created;
    try {
      ...
      if (e.code === 'P2002') {
        // Race condition: another checkUser call just created this user
        created = await db.user.findUnique({ where: { clerkId: clerkUser.id } });
      } else {
        throw e;
      }
    }
